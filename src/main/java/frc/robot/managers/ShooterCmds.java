...
public class ShooterCmds {
     ...
     * This should be called with a toggleOnTrue so that it continuously runs on
     * toggle
     * 
     * @param shootAtSpeaker A BooleanSupplier that returns true if the robot should
     *                       shoot at the
     *                       speaker, and false otherwise.
     * @param robotPose      The `robotPose` parameter represents the supplier of
     *                       the current pose
    ...
    /**
     * Determines if the pivot and shooter is at its target for shooting with a
     * small
     * tolerance
     * 
     * @return The method is returning a BooleanSupplier that returns true
     *         if the pivot and shooter are at their target states for shooting and
     *         false otherwise
     */
    public BooleanSupplier readyToShootSupplier() {
        return () -> pivot.getAtDesiredAngle()
